...
interface JobMatchesProps {
  ...
  onMatchesFound: (data: { matches: JobWithMatch[]; stats: MatchingStats }) => void;
}

export default function JobMatches({ resumeId, matches, onMatchesFound }: JobMatchesProps) {
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span className="text-sm font-normal text-gray-500 ml-2">
            {matches.length > 0 ? `(${matches.length} matches found)` : "(Upload resume to see matches)"}
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {matches.length === 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">Upload your resume to discover perfectly matched job opportunities</p>
          </div>
        ) : (
          // Matching Results
            ...
            {matches.map((match, index) => (
              <div
                ...
                className={`border rounded-lg p-6 ${
                  index === 0 ? 'border-secondary/20 bg-secondary/5' : 'border-gray-200'
                }`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-4">
                    <div className={`w-12 h-12 bg-gradient-to-r ${getCompanyGradient(match.company)} rounded-lg flex items-center justify-center flex-shrink-0`}>
                      <span className="text-white font-bold text-sm">
                        {getCompanyInitials(match.company)}
                      </span>
                    </div>
                    <div>
                      <h3 className="font-semibold text-slate-custom text-lg">{match.title}</h3>
                      <p className="text-gray-600 font-medium">{match.company}</p>
                      <p className="text-gray-500 text-sm">
                        {match.location} • {match.workType} • {formatSalary(match.salaryMin || undefined, match.salaryMax || undefined)}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={`px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1 ${getMatchColor(match.matchScore || 0)}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {match.matchingSkills?.map((skill) => (
                        <Badge key={skill} className={
                          index === 0 ? "bg-secondary/20 text-secondary" : "bg-primary/20 text-primary"
                        }>
                          {skill}
                        </Badge>
                      ))}
